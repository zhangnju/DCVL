...
public:
  ...
  void Finalize() override {
    for (auto i = 0; i < senders_.size(); ++i) {
      if (i != rank_) {
        int linger = 0;
        if(zmq_setsockopt(senders_[i].socket, ZMQ_LINGER, &linger, sizeof(linger)) != 0)
			return;
    ...
    int linger = 0;
    if(zmq_setsockopt(receiver_.socket, ZMQ_LINGER, &linger, sizeof(linger)) != 0)
		return;
    int rc = zmq_close(receiver_.socket);
    if(rc != 0)
		return;

    std::cout<<"zmq finalize: before close context";
    if(zmq_ctx_shutdown(context_)!=0)
		return;
    if(context_==nullptr)
		return;
